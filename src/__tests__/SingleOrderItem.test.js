...
import { fakeOrderItems, fakeOrderItem } from "../testUtils/fakeTestData";
...
describe("<SingleOrderItem/>", () => {
  let wrapper, myCookies, singleOrderItem;
  ...
  beforeEach(async () => {
    ...
    wrapper = mount(
      <MemoryRouter>
        <MockedProvider>
          <SingleOrderItem item={fakeOrderItem()} key={"skdjflskdjf"} />
        </MockedProvider>
      </MemoryRouter>
    );
    ...
    wrapper.update();
    singleOrderItem = wrapper.find("SingleOrderItem");
  });

  it("renders and matches snapshot", async () => {
    console.log(singleOrderItem.debug());
    expect(toJSON(singleOrderItem)).toMatchSnapshot();
  });
});
